package ejercicio.entities;

// default package
// Generated 23 Oct 2021, 18:15:01 by Hibernate Tools 4.3.5.Final

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;

/**
 * Empleado generated by hbm2java
 */
@Entity
@Table(name = "empleado", catalog = "empresa")
public class Empleado implements java.io.Serializable {

	private int codigo;
	private String apellido1;
	private String apellido2;
	private int codDepartamento;
	private String direccion;
	private String fechaNacimiento;
	private String lugarNacimiento;
	private String nombre;
	private String puesto;
	private String telefono;

	public Empleado() {
	}

	public Empleado(int codigo, String apellido1, String apellido2, int codDepartamento, String direccion,
			String fechaNacimiento, String lugarNacimiento, String nombre, String puesto, String telefono) {
		this.codigo = codigo;
		this.apellido1 = apellido1;
		this.apellido2 = apellido2;
		this.codDepartamento = codDepartamento;
		this.direccion = direccion;
		this.fechaNacimiento = fechaNacimiento;
		this.lugarNacimiento = lugarNacimiento;
		this.nombre = nombre;
		this.puesto = puesto;
		this.telefono = telefono;
	}

	@Id

	@Column(name = "codigo", unique = true, nullable = false)
	public int getCodigo() {
		return this.codigo;
	}

	public void setCodigo(int codigo) {
		this.codigo = codigo;
	}

	@Column(name = "apellido1", nullable = false, length = 50)
	public String getApellido1() {
		return this.apellido1;
	}

	public void setApellido1(String apellido1) {
		this.apellido1 = apellido1;
	}

	@Column(name = "apellido2", nullable = false, length = 50)
	public String getApellido2() {
		return this.apellido2;
	}

	public void setApellido2(String apellido2) {
		this.apellido2 = apellido2;
	}

	@Column(name = "cod_departamento", nullable = false)
	public int getCodDepartamento() {
		return this.codDepartamento;
	}

	public void setCodDepartamento(int codDepartamento) {
		this.codDepartamento = codDepartamento;
	}

	@Column(name = "direccion", nullable = false, length = 100)
	public String getDireccion() {
		return this.direccion;
	}

	public void setDireccion(String direccion) {
		this.direccion = direccion;
	}

	@Column(name = "fecha_nacimiento", nullable = false, length = 8)
	public String getFechaNacimiento() {
		return this.fechaNacimiento;
	}

	public void setFechaNacimiento(String fechaNacimiento) {
		this.fechaNacimiento = fechaNacimiento;
	}

	@Column(name = "lugar_nacimiento", nullable = false, length = 50)
	public String getLugarNacimiento() {
		return this.lugarNacimiento;
	}

	public void setLugarNacimiento(String lugarNacimiento) {
		this.lugarNacimiento = lugarNacimiento;
	}

	@Column(name = "nombre", nullable = false, length = 50)
	public String getNombre() {
		return this.nombre;
	}

	public void setNombre(String nombre) {
		this.nombre = nombre;
	}

	@Column(name = "puesto", nullable = false, length = 15)
	public String getPuesto() {
		return this.puesto;
	}

	public void setPuesto(String puesto) {
		this.puesto = puesto;
	}

	@Column(name = "telefono", nullable = false, length = 10)
	public String getTelefono() {
		return this.telefono;
	}

	public void setTelefono(String telefono) {
		this.telefono = telefono;
	}

	@Override
	public String toString() {
		return "Empleado [codigo=" + codigo + ", apellido1=" + apellido1 + ", apellido2=" + apellido2
				+ ", codDepartamento=" + codDepartamento + ", direccion=" + direccion + ", fechaNacimiento="
				+ fechaNacimiento + ", lugarNacimiento=" + lugarNacimiento + ", nombre=" + nombre + ", puesto=" + puesto
				+ ", telefono=" + telefono + "]";
	}
	
	

}
